...
void GenerateTextures()
{
	...
	GLboolean error = load_targa("blonde_white_skin_guy.tga", width, height, format, type, pixels);

	// b³¹d odczytu pliku
	if (error == GL_FALSE)
	{
		printf("Niepoprawny odczyt pliku blonde_white_skin_guy.tga");
		exit(0);
	}
	...
	error = load_targa("blonde_white_skin_guy_gray.tga", width, height, format, type, pixels);

	// b³¹d odczytu pliku
	if (error == GL_FALSE)
	{
		printf("Niepoprawny odczyt pliku blonde_white_skin_guy_gray.tga");
		exit(0);
	}
